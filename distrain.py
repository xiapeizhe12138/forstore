...
def make_data_loader(spec, tag=''):
    ...
    if tag == 'train':
        loader = torch.utils.data.DataLoader(
            dataset=dataset, batch_size=spec['batch_size'], shuffle=False, num_workers=0,
...
def train(train_loader, model, optimizer, epoch, dim2):
    model.train()
    loss_fn = nn.L1Loss()
    ...
    for i, batch in tqdm(enumerate(train_loader), leave=False, desc='train'):
...
def main(config_, save_path, rank, world_size, gpu_id=None):
    print('start main')
    ...
    # if n_gpus > 1:
    #     model = nn.parallel.DataParallel(model, device_ids=gpu_id)
    ...
    if rank % world_size == 0 and False:
    ...
    for epoch in range(epoch_start, epoch_max + 1):
        ...
        train_loss = train(train_loader, model, optimizer, epoch, dim2)
...
def _init_dist_pytorch(backend, **kwargs):
    ...
    dist.init_process_group(backend=backend,  timeout=datetime.timedelta(seconds=640000), **kwargs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--launcher', choices=['pytorch', ], default='none', help='job launcher')
    ...
    seed = None#args.get('manual_seed')
    if seed is None:
    ...
    main(config, save_path, rank, world_size, gpu_id=gpu)